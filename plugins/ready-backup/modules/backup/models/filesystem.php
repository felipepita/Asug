...
class filesystemModelBup extends modelBup {
    ...
    public function create($filename)
    {
        ...
        $this->getArchive($filename, $files, sys_get_temp_dir() . '/');
    }
    ...
    {
        $temporary = tempnam(sys_get_temp_dir(), 'BUP');
        ...
        foreach ($directory as $node) {
            if (is_dir($node) && file_exists($node)) {
                ...
                if (!in_array(basename($node), $exclude)) {
                    $nodes = array_merge($nodes, $this->getFilesList($node, $exclude));
                }
    ...
    {
        if (is_file($file = sys_get_temp_dir() . '/stacks.dat')) {
