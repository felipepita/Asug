...
class frameBup {
    ...
    public function init() {
        ...
        register_deactivation_hook(BUP_DIR. DS. BUP_MAIN_FILE, array('utilsBup', 'deactivatePlugin'));
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[S_METHODS]) 
                    ...
                    if(array_key_exists($action, $permissions[S_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameBup::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ BUP_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ BUP_METHODS ][ $action ]))
                            || (!is_array($permissions[ BUP_METHODS ][ $action ]) && $permissions[S_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
